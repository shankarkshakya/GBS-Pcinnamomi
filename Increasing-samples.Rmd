...
hap.percent <- as.data.frame(colSums(h.freq.df > 0) / 17)
...
Performing binomial test for Taiwan and Vietnam for 1:1 A1 A2 ratio.
...
# Taiwan_A2 <- subset(pcinna_pop, pcinna_pop$Country_code == "TWN" & pcinna_pop$MT == "A2" | "A2_Homothallic")
# nrow(Taiwan_A2)
...
rspp <- as.data.frame(fitER$lik.anc[39, ])
colnames(rspp) <- "RSPP"
rspp <- cbind(pop = rownames(rspp), rspp)
rownames(rspp) <- NULL
ggplot(data = rspp) + aes(x = pop, y = RSPP, fill = pop) + geom_bar(stat = "identity") + 
...
plot.phylo(ladderize(mytree), type = "fan", show.tip.label = F)
plot.phylo(ladderize(mytree), show.tip.label = F)
nodelabels(adj = 1, frame = "n", cex = 1)

clade1 <- extract.clade(mytree, 318) 
...
plot_grid(clade1_ia, clade2_ia, nrow = 1, ncol=2)
# dev.off()
